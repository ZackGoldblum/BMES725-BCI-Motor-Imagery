...
recording_time = 5  # 5 second recordings

MI_CLASSES = ["left", "right"]

def to_one_sec(eeg_data, total_num_sec=5, samp_freq=250):
    """
    Segment EEG data into one second intervals
    """
    one_sec_data = []
    for i in range(total_num_sec):
...
def prepare_data(data_dir):
    """
    Load and format EEG data for MI class labeling 
    """
    training_data_dict = {}
    ...
    for mi_class in MI_CLASSES:
        ...
        for filename in os.listdir(mi_class_dir):
            if "filtered" in filename:  # load filtered EEG data only
                eeg_data = np.loadtxt(os.path.join(mi_class_dir, filename), delimiter=',')  # (250, 8) EEG data
                one_sec_list = to_one_sec(eeg_data, total_num_sec=recording_time, samp_freq=250)
    ...
    for mi_class in MI_CLASSES:
        ...
        training_data_dict[mi_class] = training_data_dict[mi_class][:min(session_count)]  # use fewest number of sessions
    ...
    for mi_class in MI_CLASSES:
        for data in training_data_dict[mi_class]:
            if mi_class == "left":
                labeled_data.append([data, [1, 0]])  # left one-hot vector
            elif mi_class == "right":
                labeled_data.append([data, [0, 1]])  # right one-hot vector

    np.random.shuffle(labeled_data)
    
    return labeled_data

labeled_data = prepare_data(data_dir=eeg_dir)
...
print("Creating testing data...")  # testing data
train_X = np.array(labeled_data_X[0:(9*3*recording_time)])
train_y = np.array(labeled_data_y[0:(9*3*recording_time)])
print("train_X: " + str(np.shape(train_X)))
print("train_y: " + str(np.shape(train_y)) + "\n")

print("Creating training data...")  # training data
test_X = np.array(labeled_data_X[(9*3*5):(12*3*recording_time)])
test_y = np.array(labeled_data_y[(9*3*5):(12*3*recording_time)])
print("test_X: " + str(np.shape(test_X)))
print("test_y: " + str(np.shape(test_y)) + "\n")

print("Creating validation data...")  # validation data
validation_X = np.array(labeled_data_X[(12*3*recording_time):])
validation_y = np.array(labeled_data_y[(12*3*recording_time):])
print("validation_X: " + str(np.shape(validation_X)))
print("validation_y: " + str(np.shape(validation_y)) + "\n")

# ShallowConvNet from https://github.com/vlawhern/arl-eegmodels
...
epochs = 50
batch_size = 25
fitted_model = model.fit(train_X, train_y, batch_size=batch_size, epochs=epochs, validation_data=(validation_X, validation_y))
